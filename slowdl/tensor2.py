...
class Tensor(object):
    def __init__(self, data: Union[List,np.array], genesis_op=None, requires_grad=False, children=()):
    ...
    def __add__(self, other):
        ...
        if self.requires_grad and other.requires_grad:
            out = Tensor(self.data + other.data, genesis_op="add", requires_grad=True)
        else:
    ...
    def __sub__(self, other):
        if self.requires_grad and other.requires_grad:
            out =  Tensor(self.data - other.data, genesis_op="sub", requires_grad=True)
        else: 
            out = Tensor(self.data - other.data)
    ...
    def dot(self, other):
        ...
        else: out = Tensor(self.data.dot(x.data))
...
if __name__ == "__main__":
    ...
    data = torch.tensor(np.array([[0,0],[0,1],[1,0],[1,1]], dtype=np.float32),requires_grad=True).float()
    w0 = torch.tensor(np.array([[1,2,3],[1,2,3]],dtype=np.float32), requires_grad=True).float()
